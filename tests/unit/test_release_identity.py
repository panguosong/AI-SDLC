"""Repository release identity is aligned on the public 3.2.2 release."""
...
def test_package_and_source_fallback_versions_are_1_0_2() -> None:
    ...
    assert metadata["project"]["version"] == "3.2.2"
    assert '__version__ = "3.2.2"' in (
        REPO_ROOT / "src" / "ai_sdlc" / "__init__.py"
    ).read_text(encoding="utf-8")
    assert '__version__ = "3.2.2"' in (REPO_ROOT / "ai_sdlc" / "__init__.py").read_text(
        encoding="utf-8"
    )
...
def test_release_workflow_defaults_target_v1_0_2() -> None:
    ...
    assert "default: v3.2.2" in release_build
    assert "default: v3.2.2" in release_smoke
    ...
    for name in ("posix-user-guide-e2e.yml", "windows-user-guide-e2e.yml"):
        ...
        assert "v3.2.2" in text, name
        assert "v1.0.5" not in text, name


def test_stable_git_install_examples_pin_v1_0_2() -> None:
    text = (REPO_ROOT / "README.md").read_text(encoding="utf-8")
    assert "git+https://github.com/panguosong/AI-SDLC.git@v3.2.2" in text
...
def test_post_install_offline_example_allows_installer_created_files() -> None:
    ...
    assert "--expected-package-version 3.2.2" in command
...
def test_offline_readme_smoke_projects_reference_parent_bundle_venv() -> None:
    ...
    assert r"..\ai-sdlc-offline-3.2.2-windows-amd64\.venv" not in text
    assert "../ai-sdlc-offline-3.2.2-<platform>/.venv" not in text
