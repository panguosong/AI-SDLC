...
def _write_existing_project_state_residue(root: Path) -> None:
...
def test_reconcile_hint_and_artifact_candidate_deduplicate_set_like_lists() -> None:
    ...
    hint = ReconcileHint(
        needed=True,
        reason="existing artifacts outpace checkpoint",
        layout="legacy_root",
        spec_dir=".",
...
def test_reconcile_checkpoint_ignores_legacy_project_state_residue(
    ...
    init_project(tmp_path)
    _write_legacy_root_artifacts(tmp_path)
    _write_existing_project_state_residue(tmp_path)
    save_checkpoint(
        tmp_path,
        Checkpoint(
            current_stage="init",
    ...
    assert project_state.project_name == "existing-project"
...
def test_detect_reconcile_hint_prefers_current_branch_direct_formal_work_item(
    ...
    (tmp_path / ".git").mkdir()
    init_project(tmp_path)
    _write_specs_dir_legacy_artifacts(tmp_path, "001-sample-work-item")
    ...
    save_checkpoint(
        ...
        Checkpoint(
            current_stage="close",
            feature=FeatureInfo(
                id="001-sample-work-item",
                spec_dir="specs/001-sample-work-item",
                design_branch="design/001-sample-work-item-docs",
                feature_branch="feature/001-sample-work-item-dev",
                current_branch="main",
            ),
        ),
    )
    ...
    save_checkpoint(
        ...
        Checkpoint(
            ...
            feature=FeatureInfo(
                ...
                design_branch="design/001-sample-work-item",
                feature_branch="feature/001-sample-work-item",
