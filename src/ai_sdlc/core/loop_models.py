...
DecisionCapability = Literal[
    "implementation-b1", "implementation-simulation-v1", "stage-simulation-v1"
]
...
class LoopRun(LoopArtifactModel):
    ...
    def _decision_identity(self) -> LoopRun:
        validate_decision_identity(
            self.loop_type, self.decision_mode, self.decision_capability
        )
        if (
            self.decision_started_at_ms is not None
            and self.decision_capability != "stage-simulation-v1"
        ):
            raise ValueError("decision-start-marker-capability-mismatch")
        if self.decision_begin_pending_digest is not None and (
            self.decision_capability != "stage-simulation-v1"
            or self.decision_started_at_ms is None
        ):
            raise ValueError("decision-begin-pending-identity-mismatch")
        return self
...
def validate_decision_identity(
    loop_type: str, mode: str, capability: str | None
) -> None:
    """能力只在创建时显式绑定；缺省字段不改变旧文档和摘要。"""

    if mode == "legacy" and capability is None:
        return
    if (
        loop_type in set(LoopType)
        and mode == "adaptive-quantified"
        and capability == "stage-simulation-v1"
    ):
        return
    if (
        loop_type == LoopType.IMPLEMENTATION
        and mode == "adaptive-quantified"
        and capability in {"implementation-b1", "implementation-simulation-v1"}
    ):
        return
    raise ValueError("decision-identity-invalid")
