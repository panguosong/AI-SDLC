...
21. **前端方案确认先于实现**：前端需求进入实现前，必须基于项目已有技术栈、架构约束、维护能力和交付目标给出一个推荐方案，并同时提供至少一个可选 / 自定义方案，等待用户明确确认。输出必须区分“规范正文”“可选建议”“已经落地”，不得把建议或未验证依赖写成项目事实。通用规则不得硬编码框架、组件库、provider 或 style pack；只有项目事实或用户明确选择才能确定具体方案。确认前不得进入 execute、不得生成前端实现代码、不得执行交付应用。正常 Agent 路径直接使用 `ai-sdlc run` 返回的 `Applicable Rules`，不要求用户手动执行 `rules show` 或 `stage show`。
