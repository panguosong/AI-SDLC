# AI-SDLC 3.2.2 中文用户指南

本指南面向第一次接触 AI-SDLC 的普通用户，对应 `v3.2.2` 发布版本。所有安装器、离线包、校验文件和安装后版本必须保持一致；仅在对应正式 Release 可用后使用这些下载链接，不用开发分支替代未就绪的制品。
...
`v3.2.2` 正式 Release 应提供以下离线资产：

- <https://github.com/panguosong/AI-SDLC/releases/download/v3.2.2/ai-sdlc-offline-3.2.2-windows-amd64.zip>
- <https://github.com/panguosong/AI-SDLC/releases/download/v3.2.2/ai-sdlc-offline-3.2.2-windows-amd64.zip.sha256>
- <https://github.com/panguosong/AI-SDLC/releases/download/v3.2.2/ai-sdlc-offline-3.2.2-macos-arm64.tar.gz>
- <https://github.com/panguosong/AI-SDLC/releases/download/v3.2.2/ai-sdlc-offline-3.2.2-macos-arm64.tar.gz.sha256>
- <https://github.com/panguosong/AI-SDLC/releases/download/v3.2.2/ai-sdlc-offline-3.2.2-linux-amd64.tar.gz>
- <https://github.com/panguosong/AI-SDLC/releases/download/v3.2.2/ai-sdlc-offline-3.2.2-linux-amd64.tar.gz.sha256>
...
$InstallRoot = Join-Path $HOME "AI-SDLC\online-v3.2.2"
$VenvRoot = Join-Path $InstallRoot ".venv"
$DownloadRoot = Join-Path $env:TEMP "ai-sdlc-v3.2.2-online"
...
$InstallerUrl = "https://raw.githubusercontent.com/panguosong/AI-SDLC/v3.2.2/packaging/install_online.ps1"
...
```powershell
$PinnedTag = "v3.2.2"
if (-not (Select-String -LiteralPath $InstallerPath -SimpleMatch $PinnedTag -Quiet)) { throw "Installer is not pinned to v3.2.2" }
...
INSTALL_ROOT="$HOME/Applications/AI-SDLC/online-v3.2.2"
...
INSTALLER_URL="https://raw.githubusercontent.com/panguosong/AI-SDLC/v3.2.2/packaging/install_online.sh"
...
```bash
PINNED_TAG="v3.2.2"
grep -F "$PINNED_TAG" "$INSTALLER_PATH" >/dev/null || { echo "Installer is not pinned to v3.2.2"; exit 1; }
...
必须看到 `3.2.2`、`Initialized AI-SDLC project`、`当前结果 / Result`、`下一步 / Next` 和推荐继续点；空目录不会出现示例业务文件。
...
适用于 64 位 Linux（`linux-amd64`）和 bash。已存在 Python 3.11+ 时，保持发行版无关的在线兼容路径；缺少 Python 3.11+ 时，自动 bootstrap 仅认证 Debian GNU/Linux 12 (bookworm) 的 amd64/x86_64 + glibc 主机。其他无 Python 的 amd64/x86_64 + glibc 主机应使用路线 6/12 的 ai-sdlc-offline-3.2.2-linux-amd64.tar.gz。非 AMD64 或非 glibc 的 Linux 主机，v3.2.2 没有兼容的 Linux 发行资产；不得使用路线 6/12 的 AMD64 离线包。需要联网访问 GitHub，当前用户应能写入 `$HOME/.local/share`；下载与在线 Git 安装源要求主机具备 CA 证书、curl 和 Git。

```bash
set -e
PROJECT_ROOT="$HOME/projects/my-new-project"
INSTALL_ROOT="$HOME/.local/share/AI-SDLC/online-v3.2.2"
VENV_ROOT="$INSTALL_ROOT/.venv"
DOWNLOAD_ROOT="$(mktemp -d)"
mkdir -p "$PROJECT_ROOT" "$INSTALL_ROOT"
test -z "$(ls -A "$PROJECT_ROOT")" || { echo "Project directory must be empty"; exit 1; }
...
必须看到 `3.2.2`、`Initialized AI-SDLC project`、`当前结果 / Result`、`下一步 / Next` 和推荐继续点，且空目录没有示例业务代码。

<!-- AI-SDLC-USER-GUIDE-STEP: recover -->
### 7. 就地恢复

已存在 Python 3.11+ 时，保持发行版无关的在线兼容路径；缺少 Python 3.11+ 时，自动 bootstrap 仅认证 Debian GNU/Linux 12 (bookworm) 的 amd64/x86_64 + glibc 主机。其他无 Python 的 amd64/x86_64 + glibc 主机应使用路线 6/12 的 ai-sdlc-offline-3.2.2-linux-amd64.tar.gz。非 AMD64 或非 glibc 的 Linux 主机，v3.2.2 没有兼容的 Linux 发行资产；不得使用路线 6/12 的 AMD64 离线包。
...
$DownloadRoot = Join-Path $HOME "Downloads\ai-sdlc-v3.2.2"
New-Item -ItemType Directory -Force -Path $ProjectRoot, $InstallRoot, $DownloadRoot | Out-Null
if ((Get-ChildItem -LiteralPath $ProjectRoot -Force).Count -ne 0) { throw "Project directory must be empty" }
```
...
$PackageName = "ai-sdlc-offline-3.2.2-windows-amd64.zip"
$PackageUrl = "https://github.com/panguosong/AI-SDLC/releases/download/v3.2.2/$PackageName"
...
$BundleRoot = Join-Path $InstallRoot "ai-sdlc-offline-3.2.2-windows-amd64"
...
应看到 `Offline installation completed`、`3.2.2`、`Initialized AI-SDLC project`、`当前结果 / Result`、`下一步 / Next` 和推荐继续点；空项目没有示例业务代码。
...
set -e
PROJECT_ROOT="$HOME/projects/my-new-project"
INSTALL_ROOT="$HOME/Applications/AI-SDLC/offline-v3.2.2"
DOWNLOAD_ROOT="$HOME/Downloads/ai-sdlc-v3.2.2"
...
PACKAGE_NAME="ai-sdlc-offline-3.2.2-macos-arm64.tar.gz"
PACKAGE_URL="https://github.com/panguosong/AI-SDLC/releases/download/v3.2.2/$PACKAGE_NAME"
...
BUNDLE_ROOT="$INSTALL_ROOT/ai-sdlc-offline-3.2.2-macos-arm64"
...
应看到 `Offline installation completed`、`3.2.2`、`Initialized AI-SDLC project`、`当前结果 / Result`、`下一步 / Next` 和推荐继续点，项目目录仍只包含初始化工件。
...
if { [ "$ARCH" != "x86_64" ] && [ "$ARCH" != "amd64" ]; } || [ "$LIBC" = "musl" ]; then
  echo "停止：v3.2.2 没有与此主机兼容的 Linux 发行资产；不得使用 ai-sdlc-offline-3.2.2-linux-amd64.tar.gz。" >&2
  exit 1
fi
if [ "$LIBC" != "glibc" ]; then
  echo "停止：无法确定此主机使用的 libc；为避免误装，未下载、解压或安装 ai-sdlc-offline-3.2.2-linux-amd64.tar.gz。" >&2
  exit 1
fi
PROJECT_ROOT="$HOME/projects/my-new-project"
INSTALL_ROOT="$HOME/.local/share/AI-SDLC/offline-v3.2.2"
DOWNLOAD_ROOT="$HOME/Downloads/ai-sdlc-v3.2.2"
mkdir -p "$PROJECT_ROOT" "$INSTALL_ROOT" "$DOWNLOAD_ROOT"
test -z "$(ls -A "$PROJECT_ROOT")" || { echo "Project directory must be empty"; exit 1; }
```
...
PACKAGE_NAME="ai-sdlc-offline-3.2.2-linux-amd64.tar.gz"
PACKAGE_URL="https://github.com/panguosong/AI-SDLC/releases/download/v3.2.2/$PACKAGE_NAME"
curl --fail --location --retry 3 --output "$DOWNLOAD_ROOT/$PACKAGE_NAME" "$PACKAGE_URL"
curl --fail --location --retry 3 --output "$DOWNLOAD_ROOT/$PACKAGE_NAME.sha256" "$PACKAGE_URL.sha256"
```
...
```bash
tar xzf "$DOWNLOAD_ROOT/$PACKAGE_NAME" -C "$INSTALL_ROOT"
BUNDLE_ROOT="$INSTALL_ROOT/ai-sdlc-offline-3.2.2-linux-amd64"
(cd "$BUNDLE_ROOT" && ./install_offline.sh --add-to-path)
MODULE_PYTHON="$BUNDLE_ROOT/.venv/bin/python"
```
...
应看到 `Offline installation completed`、`3.2.2`、`Initialized AI-SDLC project`、`当前结果 / Result`、`下一步 / Next` 和推荐继续点；空目录未写入示例业务文件。
...
应看到 `3.2.2`、`Initialized AI-SDLC project`、`接入已有项目：已生成桥接结果`、`原任务文件不会被修改`、`当前结果 / Result`、`下一步 / Next` 和推荐继续点；Git 差异只应包含用户确认的 AI-SDLC 工件。
...
输出应包含 `3.2.2`、`Initialized AI-SDLC project`、`接入已有项目：已生成桥接结果`、`原任务文件不会被修改`、`当前结果 / Result`、`下一步 / Next` 和推荐继续点。
...
适用于 `linux-amd64`。已存在 Python 3.11+ 时，保持发行版无关的在线兼容路径；缺少 Python 3.11+ 时，自动 bootstrap 仅认证 Debian GNU/Linux 12 (bookworm) 的 amd64/x86_64 + glibc 主机。其他无 Python 的 amd64/x86_64 + glibc 主机应使用路线 6/12 的 ai-sdlc-offline-3.2.2-linux-amd64.tar.gz。非 AMD64 或非 glibc 的 Linux 主机，v3.2.2 没有兼容的 Linux 发行资产；不得使用路线 6/12 的 AMD64 离线包。在已有项目根目录使用 bash 并保存当前工作，确认当前用户可写安装目录。
...
应看到 `3.2.2`、`Initialized AI-SDLC project`、`接入已有项目：已生成桥接结果`、`原任务文件不会被修改`、`当前结果 / Result`、`下一步 / Next` 和推荐继续点。
...
必须看到 `Offline installation completed`、`3.2.2`、`Initialized AI-SDLC project`、`接入已有项目：已生成桥接结果`、`原任务文件不会被修改`、`当前结果 / Result`、`下一步 / Next` 和推荐继续点。

<!-- AI-SDLC-USER-GUIDE-STEP: recover -->
### 7. 就地恢复

`SHA256 verification failed` 时停止并重新获取包与 sidecar。权限错误使用单次 Bypass；Windows 运行时目录内的 direct `Scripts\ai-sdlc.exe` 活动时不能安全原地替换；显式 direct self-update 零安装并返回非零，请改用 `python -m ai_sdlc`（本路线即 `& $ModulePython -m ai_sdlc self-update install --version 3.2.2`）或新终端中的 stable `ai-sdlc`。命令不可用时使用 `& $ModulePython -m ai_sdlc status`，`No module named ai_sdlc` 时重跑 `install_offline.ps1 -AddToPath`。若 Git 显示非预期业务变化，停止检查。`open gates`、代理和 Shell 问题分别按 CLI 指引、`ai-sdlc adapter select`、`ai-sdlc adapter shell-select` 处理。
...
应看到 `Offline installation completed`、`3.2.2`、`Initialized AI-SDLC project`、`接入已有项目：已生成桥接结果`、`原任务文件不会被修改`、`当前结果 / Result`、`下一步 / Next` 和推荐继续点。
