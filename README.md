...
> 本文为 `v3.2.2` 发布文档；安装前请确认对应正式 Release 和目标平台资产已经可用，未发布时不要用开发分支替代正式安装来源。第一次使用时，请在下面的 12 条路线中只选择一条完整执行；从 `v2.0.0` 升级前请阅读 [v3 迁移说明](docs/v3-migration.zh-CN.md)，从 `v1.0.2` 跨大版本升级还应先阅读 [v2 迁移说明](docs/v2-migration.zh-CN.md)。
...
Linux 选择边界：已存在 Python 3.11+ 的 Linux 主机保持发行版无关的在线兼容路径。缺少 Python 时，在线自动 bootstrap 仅认证 Debian GNU/Linux 12 (bookworm) + amd64/x86_64 + glibc；其他缺少 Python 的 amd64/x86_64 + glibc Linux 主机使用路线 6/12 的 ai-sdlc-offline-3.2.2-linux-amd64.tar.gz。非 AMD64 或非 glibc 的 Linux 主机没有兼容的 v3.2.2 Linux 发行资产，不得使用路线 6/12 的 AMD64 离线包。
...
```powershell
python -m pip install "git+https://github.com/panguosong/AI-SDLC.git@v3.2.2"
ai-sdlc --version
```
...
离线包会包含 AI-SDLC wheel、依赖 wheel、安装脚本、包内 `SHA256SUMS` 校验清单和可选的 Python 运行时。每个正式压缩包同时发布同名 `.sha256` 文件。`v3.2.2` 的目标发行产物名称如下；下载前须核对正式 Release 已提供这些文件及匹配摘要：
