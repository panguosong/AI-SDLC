...
本次源码与发布文档目标版本为 `3.1.0` / `v3.1.0`。下列命令用于发布前候选验证或正式制品复验；公开可用状态以正式 Release 为准。
...
离线包包含与源码相同的新实例五 Loop 量化能力；安装后仍从普通 `ai-sdlc run` 的 Next 交由宿主准备候选与独立判断，不要求用户手填 JSON 或预算。离线安装不等于附带离线 AI provider，也不免除实际浏览器、测试、审查与普通 Close 门禁。使用边界见 [用户指南](../../USER_GUIDE.zh-CN.md#用-v310-推进量化-loop)。
...
复验正式发布的 `v3.1.0` 源码时，固定使用该 tag；正式 tag 尚不可用时不要执行此下载步骤或替换为开发分支：
...
3.1.0 候选构建只能在已审查 PR 的当前工作树根目录执行：
...
```powershell
python packaging/offline/verify_offline_bundle.py <bundle-dir> --require-bundled-runtime --require-checksums --expected-package-version 3.1.0 --archive-checksum <archive> <archive>.sha256
```

安装 smoke 后补充安装日志：

```powershell
python packaging/offline/verify_offline_bundle.py <bundle-dir> --require-bundled-runtime --expected-package-version 3.1.0 --archive-checksum <archive> <archive>.sha256 --install-log <install-log>
```
...
```powershell
$Bundle = "ai-sdlc-offline-3.1.0-windows-amd64"
Set-Location $Bundle
...
```bash
cd ai-sdlc-offline-3.1.0-<platform>
...
工作流默认验证 `v3.1.0`；只有普通跨平台 smoke 和发布检查全部通过后才上传 Draft Release，发布后还必须重复制品 smoke。

## 交付要求

- 包版本、目录名、manifest 和 wheel 版本均为 `3.1.0`；
