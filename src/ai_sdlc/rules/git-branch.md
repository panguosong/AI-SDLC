...
- `design/<WI>-docs` 是受支持的可选命名；新分支默认使用 `feature/<WI>-docs`。
...
当使用 codex/*、临时 topic 分支或 git worktree 时：
  ...
  4. 未关联到当前 work item 的 scratch/worktree 分支，至少要在 inventory 中可见
```
...
当项目已初始化（宪章存在），处理新 Feature 时：
  1. 确认在 main 分支上
  2. git pull（如有远程仓库）
  3. 创建 feature/<WI>-docs（也支持 design/<WI>-docs）
  4. 后续流程同标准流程
```
