...
def test_online_installers_default_to_the_exact_public_git_tag(tmp_path: Path) -> None:
    expected = "git+https://github.com/SinclairPan/Ai_AutoSDLC.git@v3.1.0"
...
def test_install_online_rejects_ubuntu_glibc_without_python_before_any_mutation(
    ...
    assert "ai-sdlc-offline-3.1.0-linux-amd64.tar.gz" in result.stdout
...
def test_install_online_preserves_independent_linux_host_facts_when_os_release_is_unknown(
    ...
    if expects_amd64_fallback:
        ...
        assert "ai-sdlc-offline-3.1.0-linux-amd64.tar.gz" not in result.stdout
...
def test_user_guide_documents_published_assets_and_two_new_user_paths() -> None:
    ...
    assert "ai-sdlc-offline-3.1.0-windows-amd64.zip" in guide
    assert "ai-sdlc-offline-3.1.0-macos-arm64.tar.gz" in guide
    assert "ai-sdlc-offline-3.1.0-linux-amd64.tar.gz" in guide
