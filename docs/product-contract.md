...
legacy 的输入摘要缺失不证明该记录历史上没有绑定。任务录入、验证、Close 及关闭后的 Frontend 消费共同核对已有执行输入路径和正式评审足迹；原生记录删除、清空、替换摘要，或修改上游路径后重算摘要，均不能取得旧格式例外。没有原生绑定足迹的真实旧 opaque 凭据继续沿用既有读取，普通关闭后正文修改仍按原规则处理。
