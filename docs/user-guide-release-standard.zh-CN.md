...
- Linux 在线路线中，已有 Python 3.11+ 的路径保持发行版无关；缺少 Python 3.11+ 的自动 bootstrap 仅认证 Debian GNU/Linux 12 (bookworm) + amd64/x86_64 + glibc。其他缺少 Python 的 amd64/x86_64 + glibc 主机必须就地指向路线 6/12 的 `ai-sdlc-offline-3.2.1-linux-amd64.tar.gz`；非 AMD64 或非 glibc 主机必须明确 v3.2.1 没有兼容的 Linux 发行资产，且不得使用路线 6/12 的 AMD64 离线包。该边界必须同时出现在每条 Linux 在线路线的 `prerequisites` 和 `recover`，不得用共享文本或“同上”替代。
...
`v3.2.1` 手册须保留安装后的普通 `run` 入口，说明 Result / Next 与宿主 Agent 的分工：`run` 只读路由，宿主按 Next 新建量化 Loop、准备冻结输入和独立判断，用户不手填 JSON、评分或预算。该说明可放在公共概念章节，不改变任何路线的七个独立步骤。
