# AI-SDLC 3.1.0 离线发布检查清单

本次目标版本为 `3.1.0` / `v3.1.0`。候选源码版本不代表已经公开发布；只有本清单的发布后验证完成，才能宣布发布完成。

## 版本与源码

- [ ] `pyproject.toml` 为 `3.1.0`；
- [ ] 两个 `ai_sdlc/__init__.py` 回退版本均为 `3.1.0`；
- [ ] `uv.lock` 中本项目版本为 `3.1.0`；
...
- [ ] 精确 main SHA 上存在 annotated tag `v3.1.0` 和全新的空 Draft Release；
...
- [ ] 发布事件触发的 Release Artifact Smoke 三平台全部通过，正式下载并安装的 CLI 均报告 `3.1.0`；
- [ ] 发布后分别手动触发现有 `windows-user-guide-e2e.yml` 和 `posix-user-guide-e2e.yml`，输入 `tag=v3.1.0`，要求全部成功；这两份流程没有 release 事件触发器，不等待自动触发；
- [ ] 所有旧 tag 对象、Release ID、旧草稿状态及资产身份保持不变；只允许 Latest 切换到 `v3.1.0`；
- [ ] 平台工作流 artifact 完整；
