...
# Env:
#   AI_SDLC_PACKAGE_SPEC=git+https://github.com/panguosong/AI-SDLC.git@v3.2.2   optional published package spec for pip install
#   PYTHON=/path/to/python3.11            optional interpreter override

set -euo pipefail

PACKAGE_SPEC="${AI_SDLC_PACKAGE_SPEC:-git+https://github.com/panguosong/AI-SDLC.git@v3.2.2}"
ADD_TO_PATH=0
...
print_unsupported_linux_host() {
  print_status \
    ...
    "Use ai-sdlc-offline-3.2.2-linux-amd64.tar.gz from User Guide route 6/12." \
    "缺少 Python 的在线自动安装仅认证 Debian GNU/Linux 12 (bookworm) + amd64/x86_64 + glibc；请使用路线 6/12 的 ai-sdlc-offline-3.2.2-linux-amd64.tar.gz。" \
    "Missing-Python online bootstrap is certified only for Debian GNU/Linux 12 (bookworm) + amd64/x86_64 + glibc; use the exact v3.2.2 Linux offline asset from route 6/12."
}
...
print_unsupported_linux_arch_or_libc() {
  print_status \
    ...
    "Use a host with a compatible v3.2.2 Linux distribution asset." \
    "缺少 Python 的在线自动安装仅认证 Debian GNU/Linux 12 (bookworm) + amd64/x86_64 + glibc；该架构或 libc 没有兼容的 v3.2.2 Linux 发行资产。" \
    "Missing-Python online bootstrap is certified only for Debian GNU/Linux 12 (bookworm) + amd64/x86_64 + glibc; no compatible v3.2.2 Linux distribution asset exists for this architecture or libc."
}
