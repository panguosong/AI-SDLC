...
class BranchManager:
    ...
    def _alternate_docs_branch_name(work_item_id: str) -> str:
        """Return the alternate docs-branch name supported by the contract."""
        return f"design/{work_item_id}-docs"

    def _resolve_docs_branch(self, work_item_id: str) -> str:
        """Resolve the preferred docs branch, then the supported alternate."""
        preferred = self._docs_branch_name(work_item_id)
        if self._git.branch_exists(preferred):
            return preferred
        alternate = self._alternate_docs_branch_name(work_item_id)
        if self._git.branch_exists(alternate):
            return alternate
        return preferred
