"""Checkpoint optional linkage fields."""
...
def test_load_checkpoint_without_link_fields(tmp_path: Path) -> None:
    cp_path = tmp_path / CHECKPOINT_PATH
    cp_path.parent.mkdir(parents=True)
    cp_path.write_text(
        """
pipeline_started_at: '2026-07-17T00:00:00+00:00'
pipeline_last_updated: '2026-07-17T00:00:00+00:00'
current_stage: init
feature:
  id: '001'
  spec_dir: specs/001
...
def test_strict_load_accepts_pipeline_block(tmp_path: Path) -> None:
    cp_path = tmp_path / CHECKPOINT_PATH
    cp_path.parent.mkdir(parents=True)
    cp_path.write_text(
        """
pipeline:
  ...
  last_updated: '2026-07-17T00:05:00+00:00'
current_stage: init
feature:
  id: '001'
  spec_dir: specs/001
...
""".strip(),
    ...
    assert loaded.pipeline_started_at == "2026-07-17T00:00:00+00:00"
    assert loaded.pipeline_last_updated == "2026-07-17T00:05:00+00:00"
...
def test_save_link_fields_roundtrip(tmp_path: Path) -> None:
    cp = Checkpoint(
        ...
        linked_wi_id="001-sample-work-item",
        linked_plan_uri=".cursor/plans/x.plan.md",
        last_synced_at="2026-07-17T12:00:00Z",
    )
    save_checkpoint(tmp_path, cp)
    loaded = load_checkpoint(tmp_path)
    assert loaded is not None
    assert loaded.linked_wi_id == "001-sample-work-item"
    assert loaded.linked_plan_uri == ".cursor/plans/x.plan.md"
    assert loaded.last_synced_at == "2026-07-17T12:00:00Z"
