...
from ai_sdlc.core.implementation_models import ImplementationReport
...
def _start_frontend_evidence_loop_locked(
    options: FrontendEvidenceStartOptions,
    *,
    review_input_validator: ReviewInputValidator | None = None,
) -> FrontendEvidenceCommandResult:
    ...
    previous_run = previous_input = None
    if artifacts.loop_run_path.is_file():
        try:
            previous_run = read_loop_run(artifacts.loop_run_path)
            previous_input = read_input(artifacts.input_path)
        except ValueError as exc:
            return _blocked_result(str(exc), loop_id=loop_id, artifacts=planned_refs)
        if not options.dry_run and (
            previous_run.decision_capability != "stage-simulation-v1"
            or options.decision_capability != "stage-simulation-v1"
            or previous_run.status == LoopStatus.CLOSED
        ):
            return _blocked_result(
    ...
    frontend_input = FrontendEvidenceInput.model_validate(
        {
            **frontend_input.model_dump(),
    ...
    if (
        ...
        and frontend_input.decision_capability == "stage-simulation-v1"
    ):
        frontend_input = frontend_input.model_copy(
...
def _skip_frontend_evidence_loop_locked(
    options: FrontendEvidenceSkipOptions,
    *,
    review_input_validator: ReviewInputValidator | None = None,
) -> FrontendEvidenceCommandResult:
...
def close_frontend_evidence_loop(
    ...
    *,
    ...
    root = options.root.resolve()
    run_path, blocker = resolve_frontend_evidence_loop_run_path(root, options.loop_id)
    if blocker:
...
def _close_frontend_evidence_loop_locked(
    options: FrontendEvidenceCloseOptions,
    *,
    review_input_validator: ReviewInputValidator | None = None,
    reviewed_artifacts: Mapping[str, bytes] | None = None,
    ...
    try:
        if reviewed_artifacts is None:
            ...
            frontend_input = FrontendEvidenceInput.model_validate_json(
                _reviewed_frontend_evidence_bytes(
                    root,
    ...
    if (
        frontend_input.loop_id != loop_run.loop_id
        or frontend_input.work_item_id != loop_run.work_item_id
    ):
        return _blocked_result(
            "Frontend-evidence input identity does not match its loop.",
            loop_id=loop_run.loop_id,
            artifacts=artifacts.refs(root),
        )
    _, _, implementation_blocker = _implementation_gate(
        root,
...
def _implementation_gate(
    ...
    *,
    ...
    try:
        ...
        return "", "", f"Implementation close artifact is not verified: {exc}"
...
def _browser_gate_freshness_blocker(
    ...
    *,
    ...
    try:
        implementation_close = read_verified_implementation_close(
            root,
            frontend_input.implementation_loop_id,
            review_input_validator=review_input_validator,
        )
...
def _build_snapshot(
    ...
    *,
    ...
    freshness_blocker = _browser_gate_freshness_blocker(
        root,
        frontend_input,
        payload,
        review_input_validator=review_input_validator,
...
def _build_loop_run(
    *,
    ...
    return LoopRun(
        ...
        decision_mode=frontend_input.decision_mode,
        decision_capability=frontend_input.decision_capability,
