...
def test_build_wheel_includes_templates_and_entry_points(tmp_path: Path) -> None:
    ...
    with zipfile.ZipFile(wheel_path) as archive:
        ...
        assert "ai_sdlc/core/review_kernel.py" in names
        assert "ai_sdlc/core/pr_review_provider.py" in names
        assert "ai_sdlc/rules/lean-code.md" not in names
        assert not any(name.startswith("ai_sdlc/core/lean_code") for name in names)
        assert not any(name.startswith("ai_sdlc/core/stage_review/") for name in names)
