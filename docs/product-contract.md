...
legacy 的输入摘要缺失不证明该记录历史上没有绑定。任务录入、验证、Close 及关闭后的 Frontend 消费共同核对已有执行输入路径和独立保存的正式评审原件；新评审原件的实施输入窄摘要与完整评审摘要来自同次捕获，不由宿主补填。原生记录删除、清空、替换摘要，或同时改写输入与运行记录后重算摘要，均不能取得旧格式例外。没有原生绑定足迹的真实旧 opaque 凭据继续沿用既有读取。新评审的窄绑定不禁止普通关闭后的正文修改；旧评审缺少该字段时，仅在原完整评审摘要仍可真实复验的条件下允许只读消费，材料漂移或依据不足则明确拒绝，不自动迁移或补造旧绑定。
