...
def _complete_route(route_id: str) -> str:
    ...
    if channel == "online" and platform == "linux-amd64":
        python_bootstrap_boundary = (
            ...
            "路线 6/12 的 ai-sdlc-offline-3.2.2-linux-amd64.tar.gz。"
            "非 AMD64 或非 glibc 的 Linux 主机，v3.2.2 没有兼容的 Linux 发行资产；"
...
def test_repository_activates_the_standard_in_v3_0_1() -> None:
    assert parse_project_version((ROOT / "pyproject.toml").read_text()) == (3, 2, 2)
