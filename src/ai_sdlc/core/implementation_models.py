...
class ImplementationReviewBinding(BaseModel):
    """旧续办关闭凭据的只读兼容；正常关闭不再创建这种绑定。"""

    model_config = ConfigDict(extra="forbid")
    review_round: int = Field(strict=True, gt=2)
    input_digest: str = Field(pattern=r"^[0-9a-f]{64}$")
    outcome_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
    continuation_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
...
class ImplementationClose(LoopArtifactModel):
    ...
    review_binding: ImplementationReviewBinding | None = Field(
        default=None, exclude_if=lambda value: value is None
    )
